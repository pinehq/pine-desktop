...
Текущий slice открывает локальные проекты через `Ctrl+Shift+O`, сохраняет
активный файл через `Ctrl+S` с защитой от внешних изменений и показывает
асинхронный Git status. Нажатие на изменённый файл в разделе Git открывает
ограниченный по размеру diff preview.
