...
Чтобы development-бинарник автоматически запускался после каждого входа в
GNOME, один раз собрать его в постоянный target-каталог и установить autostart
entry:

```sh
CARGO_TARGET_DIR="$HOME/.cache/pine-target" cargo build -p pine-linux
./dev/lima/install-autostart.sh
```

Инсталлятор вычисляет пути текущего пользователя и проекта, поэтому desktop
entry не привязан к конкретному имени macOS- или Linux-пользователя.

Codex, Claude Code и Pi устанавливаются шаблоном VM автоматически. В уже
созданном госте их можно установить или обновить той же командой:

```sh
./dev/lima/install-agents.sh
```

После установки выполнить `codex`, `claude` и `pi` во встроенном терминале и
пройти авторизацию каждого CLI. Токены и API-ключи в проект не записываются.
...
занимает заметное время. Шаблон включает natural scrolling для мыши и touchpad и
отключает блокировку экрана тестовой VM.

Шаблон также декларативно подключает APT-репозиторий проекта и устанавливает
релизный пакет `pine` (бинарник `/usr/bin/pine`, desktop entry и иконка),
чтобы smoke-проверка шла по тому же каналу, что и у пользователей. В уже
созданном госте переустановить или обновить релизный пакет можно той же
командой из примонтированного каталога проекта:

```sh
./dev/lima/install-pine-apt.sh
```

Релизный пакет и dev-сборка из примонтированного дерева сосуществуют:
autostart-entry из `install-autostart.sh` указывает на dev-бинарник, а пакет
ставит `/usr/bin/pine`. После автоматического входа открыть
Terminal и выполнить в примонтированном каталоге проекта:
